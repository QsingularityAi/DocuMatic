...
import { WorkOrderCompletedEmailData } from '@/types/emails';

export const workOrderCompletedTemplate = (
  data: WorkOrderCompletedEmailData,
  forceDarkMode = false,
): string => {
  const template: EmailTemplate = {
    peak: `${data.peak}`,
    content: `
    <p><img src="https://cdn.documatic.xyz/icon-success.png" width="48" style="width: 48px; height: 48px;" /></p>
    ...
    <p class="dark-mode-text-base" style="margin: 0; margin-bottom: 24px; color: $colors.light.text;">
      The Work Order #${data.workOrderId} "<strong>${data.workOrderName}</strong>" has been completed by <strong>${data.completedBy.name}</strong>.
    </p>

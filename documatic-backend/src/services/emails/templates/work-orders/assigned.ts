...
import { emailLayout, EmailTemplate } from '@/services/emails/templates/layout';
import { WorkOrderAssignedEmailData } from '@/types/emails';

export const workOrderAssignedTemplate = (
  data: WorkOrderAssignedEmailData,
  forceDarkMode = false,
): string => {
  const template: EmailTemplate = {
    ...
    <p class="dark-mode-text-base" style="margin: 0; margin-bottom: 24px; color: $colors.light.text;">
      You’ve been assigned the work order #${data.workOrderId} "<strong>${data.workOrderName}</strong>" by <strong>${data.assignedBy.name}</strong>.
    </p>
